...
def main():
    p=argparse.ArgumentParser();p.add_argument('--manifest',type=Path,required=True);p.add_argument('--events',type=Path,required=True);p.add_argument('--results',type=Path,required=True);p.add_argument('--require-records',action='store_true');p.add_argument('--block-oracle',action='store_true');a=p.parse_args()
    if os.geteuid()!=0 or run(['systemd-detect-virt','--vm','--quiet'],False).returncode: raise SystemExit('Root in disposable VM required')
    devices=json.loads(run(['lsblk','-J','-b','-o','PATH,SIZE,RO,FSTYPE,SERIAL']).stdout)['blockdevices']
    disks=[d for d in devices if d.get('serial')=='winluks-cut' and d['ro']]
    assert len(disks)==1
    parts=[d for d in disks[0]['children'] if d.get('fstype')=='ntfs' and d['ro']]
    ...
    with tempfile.TemporaryDirectory(prefix='winluks-ntfs-ro-') as tmp:
        run(['ntfs-3g','-o','ro',part['path'],tmp])
        try: shutil.copyfile(Path(tmp)/'winluks-lab/powercut/volume.img',image)
    ...
    try:
        if a.block_oracle:
            ...
            with open(dev,'rb') as actual,(a.manifest.parent/m['plaintext']).open('rb') as original:
                position=0;total=int(meta['segments']['0'].get('size','0')) if meta['segments']['0'].get('size')!='dynamic' else image.stat().st_size-offset
            ...
            result.update(block_oracle=True,acknowledged_block_writes=3,bytes_compared=position,required_checks_passed=True)
