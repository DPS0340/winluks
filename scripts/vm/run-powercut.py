...
"""
import argparse,base64,json,os,re,select,shutil,signal,socket,subprocess,time
...
def main():
    ...
    child=pexpect.spawn('ssh',['-tt',*common,'-p','22281','lab@127.0.0.1',r'C:\winluks-lab\bin\winluks2.exe open --image C:\winluks-lab\powercut\volume.img --keyslot 0 --filesystem btrfs --read-write'],encoding='utf-8',timeout=90)
    ...
    if not a.case.endswith('idle'):
        ...
        work=subprocess.Popen(ps_cmd(script),stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    ...
    report={'case':a.case,'trial':a.trial,'cut':'SIGKILL of verified trial QEMU PID','hard_cut':True,'events':events}
    ...
    if a.case in ['normal-close','power-close','disk-full']:
        ...
        if a.case!='power-close':
            child.expect(pexpect.EOF,timeout=90);child.close()
            report['close_exit']=child.exitstatus
    ...
    log.close();text=(out/'console.txt').read_text();assert key not in text and key.encode() not in worklog
    ...
    time.sleep(2)
    required=' --block-oracle' if a.case=='raw-block' else (' --require-records' if a.case=='normal-close' else '')
